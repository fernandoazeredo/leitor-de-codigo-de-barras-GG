...
export function ConferenciaPedidos(){
  ...
  useEffect(()=>{
    void listarPedidosConferencia().then(rows=>setPedidos(rows)).finally(()=>setCarregando(false));
  },[]);
  ...
  async function importarArquivos(files:FileList|File[]){
    ...
    const atuais=await listarPedidosConferencia();
    const ids=new Set(atuais.map(p=>p.id));
    const novos:PedidoConferencia[]=[];

    for(const file of xmls){
      try{
        const novo=parseNFeXml(await file.text(),file.name);
        if(ids.has(novo.id)){duplicados++;continue;}
        ids.add(novo.id);
        novos.push(novo);
        await salvarPedidoConferencia(novo);
        criados++;
    ...
    const partes=[`${criados} card(s) criado(s)`];
    if(duplicados)partes.push(`${duplicados} NF(s) já existente(s)`);
    if(erros)partes.push(`${erros} arquivo(s) com erro`);
  ...
  async function conferir(cod:string){
    ...
    if(direto){
      ...
      await persistOne(atualizado);
      const sit=situacaoItem(itens.find(i=>i.id===direto.id)!);
      setMensagem(sit==='OK'?`✅ CONFERIDO — ${direto.descricao}`:sit==='FALTANDO'?`⚠️ FALTANDO — ${direto.descricao}`:`🔺 DIVERGÊNCIA — quantidade excedida de ${direto.descricao}`);
      return;
    }

    const identificado=await identificarNaBase(c);
    if(!identificado){setMensagem('🔺 DIVERGÊNCIA — produto não identificado ou não pertence a esta NF.');return;}
    ...
    if(candidato && candidato.score>=70){
      ...
      if(item){
        ...
        await persistOne({...pedido,itens,status:'EM_CONFERENCIA'});
        setMensagem(`⚠️ ATENÇÃO — código associado por semelhança a ${item.descricao}. Confira visualmente.`);
        return;
      }
    }
  ...
  async function removerUltimo(itemId:string){
    if(!pedido)return;
    const itens=pedido.itens.map(i=>i.id===itemId?{...i,conferido:Math.max(0,i.conferido-1)}:i);
    await persistOne({...pedido,itens});
  }

  async function foto(file?:File){
    if(!file)return;
    try{setSelfie(await compactarSelfie(file));}catch(e){setMensagem(e instanceof Error?e.message:'Não foi possível registrar a selfie.');}
  }

  async function autorizar(){
    if(!pedido||!selfie)return;
    await persistOne({...pedido,autorizacao:{selfie,dataHora:new Date().toISOString()},status:'LIBERADO_AUTORIZACAO'});
    setMensagem('🔐 LIBERADO COM AUTORIZAÇÃO');
    setSelfie('');
  }
  ...
  return <section className="orders-module">
    {!pedido&&<>
      ...
      <label
        className={`xml-drop-zone ${arrastando?'dragging':''}`}
        onDragOver={e=>{e.preventDefault();setArrastando(true);}}
        onDragLeave={()=>setArrastando(false)}
        onDrop={e=>{e.preventDefault();setArrastando(false);void importarArquivos(e.dataTransfer.files);}}
      >
        <div className="drop-icon">⇩</div>
        <b>ARRASTE AS NOTAS XML AQUI</b>
        <span>Uma NF-e = um card de pedido</span>
        <small>Ou clique para selecionar vários XML de uma vez</small>
      ...
      {carregando&&<div className="card"><b>Carregando pedidos...</b></div>}
      ...
      <div className="orders-grid">{pedidos.map(p=>{
        const faltando=p.itens.reduce((s,i)=>s+Math.max(0,i.quantidade-i.conferido),0);
        const excedente=p.itens.some(i=>i.conferido>i.quantidade);
        return <button className={`order-card status-${p.status.toLowerCase()}`} key={p.id} onClick={()=>{setAberto(p.id);setMensagem('');}}>
          <div className="order-card-head"><strong>NF {p.numeroNF}</strong><span>{p.status==='CONFERIDO'?'✅':p.status==='LIBERADO_AUTORIZACAO'?'🔐':excedente?'🔺':'⚠️'}</span></div>
          <b>{p.cliente}</b>
          <small>{p.itens.length} itens · {faltando>0?`${faltando} pendente(s)`:'quantidades completas'}</small>
          <small>{p.status.replaceAll('_',' ')}</small>
          {p.autorizacao&&<div className="auth-mini-row"><img className="auth-selfie-mini" src={p.autorizacao.selfie} alt="Autorização"/><span>Liberado com autorização</span></div>}
        </button>;
      })}</div>
    </>}
    ...
    {pedido&&<>
      ...
      <div className="card order-summary">
        <div className="order-card-head"><div><h2>NF {pedido.numeroNF}</h2><b>{pedido.cliente}</b></div><span className="big-status">{pedido.status==='CONFERIDO'?'✅':pedido.status==='LIBERADO_AUTORIZACAO'?'🔐':pedido.itens.some(i=>i.conferido>i.quantidade)?'🔺':'⚠️'}</span></div>
      ...
      <div className="order-items">{pedido.itens.map(item=>{
        ...
        return <div className={`card order-item item-${sit.toLowerCase()}`} key={item.id}>
          ...
          <small>{sit==='OK'?'CONFERIDO':sit==='FALTANDO'?`FALTAM ${Math.max(0,item.quantidade-item.conferido)}`:`DIVERGÊNCIA · EXCEDENTE ${item.conferido-item.quantidade}`}</small>
          {item.conferido>0&&<button className="mini-link" onClick={()=>void removerUltimo(item.id)}>Desfazer 1 leitura</button>}
        </div>;
      })}</div>

      {pedido.status!=='CONFERIDO'&&pedido.status!=='LIBERADO_AUTORIZACAO'&&<div className="card authorization-card">
        <h3>Liberação com divergência</h3>
        <p className="muted">Se o responsável autorizar o carregamento com falta, incompleto ou divergência, registre a selfie para liberar.</p>
        <label className="button">📷 Tirar selfie do responsável<input hidden type="file" accept="image/*" capture="user" onChange={e=>void foto(e.target.files?.[0])}/></label>
        {selfie&&<img className="auth-selfie-preview" src={selfie} alt="Prévia da autorização"/>}
        <button className="primary wide" disabled={!selfie} onClick={()=>void autorizar()}>Liberar com autorização</button>
      </div>}
